...
observaciones_Estado_Esqueleto, observaciones_Nivel_Aceite_Motor, observaciones_Nivel_Aceite_Hidraulico, observaciones_Nivel_Anticongelante,
observaciones_Baterias, observaciones_Luces, observaciones_Cadena_Avance, observaciones_Banda_Alternador_Ventilador,
observaciones_Fugas_Maquina_Encendida, observaciones_Movimientos_Velocidad_Maquina_Trabajando, observaciones_Presion_Motor_50_PSI_Maquina_Trabajando,
observaciones_Temperatura_Motor_100_180_Maquina_Trabajando, observaciones_Horometro_Inicial, observaciones_Horometro_Final,
observaciones_Adicionales
FROM Excavadora 
WHERE (Maquina_ID = '11' OR Maquina_ID = '12' OR Maquina_ID = '13') AND Turno = 'Vespertino'
...
while ($row = mysqli_fetch_assoc($result)) {
    $aspectos = array(
        $row['Estado_Esqueleto'], $row['Nivel_Aceite_Motor'], $row['Nivel_Aceite_Hidraulico'], $row['Nivel_Anticongelante'], $row['Baterias'],
        $row['Luces'], $row['Cadena_Avance'], $row['Banda_Alternador_Ventilador'], $row['Fugas_Maquina_Encendida'],
        $row['Movimientos_Velocidad_Maquina_Trabajando'],
        $row['Presion_Motor_50_PSI_Maquina_Trabajando'], $row['Temperatura_Motor_100_180_Maquina_Trabajando'],
...
foreach ($aspectos as $index => $aspecto) {
    ...
    $pdf->SetFont('helvetica', 'B', 12);
    if ($index == 8) {
        $pdf->Cell($pageWidth - $pdf->getMargins()['left'] - $pdf->getMargins()['right'], 9, 'Maquina encendida', 0, 1, 'C', 0, '', 0, false, 'T', 'M');
    } else if ($index == 9) {
