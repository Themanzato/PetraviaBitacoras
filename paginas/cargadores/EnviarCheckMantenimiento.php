...
session_start();
//SI NO HAY username de la sesion, mensaje de error, CANCELAR EL REPORTE Y CERRAR SESION 
...
|| empty($horometroFinal) */
    ...
    $sql = "CALL insercionCheck(
        ...
        $sqlCargador = "CALL InsertarDatosCargador(
            '$nombre_maquina', 
            '$nombreUsuario',
            '$turno',
            '$estadoEsqueleto',
                ...
                if ($conn->query($sql) === TRUE) {
                    ...
                    if ($conn->query($sqlCargador) === TRUE) {
                        if ($conn->query($sqlReportes) === TRUE) {
                            // Muestra el mensaje de advertencia con JavaScript
                            echo '<script>
                            alert("Registro guardado correctamente.");
                            window.location.href = "cargadores_principal.php";
                          </script>';
                        } else {
                            // Manejar errores en la tercera consulta
